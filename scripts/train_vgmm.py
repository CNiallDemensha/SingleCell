...
from model import GMM, ConditionalGMM, FB_GMM, Variational_GMM
from PIL import Image
...
model = Variational_GMM(params.k, train_data.shape[1]).to(device)
...
for e in range(start_epoch, params.epochs):
    ...
    for i in tqdm(range(0, train_data.shape[0], params.batch_size)):
        ...
        elbo, log_p_x_given_alpha_z, kl_alpha_beta, kl_q_pi, kl_Q_PI, pred, alpha_posterior_prob, alpha_posterior_prob_sample, z_prob = model(batch_data, T[e])
        beta_prior_prob = torch.sigmoid(5 * model.beta_prior)
    ...
    writer.add_image("alpha_posterior_prob_sample", alpha_posterior_prob_sample[None, :, :], e)
    writer.add_image("beta_prior_prob", torch.sigmoid(5 * model.beta_prior[None, :, :]), e)
    ...
    writer.add_scalar("v_scores", np.mean(v_scores), e)
    ...
    #print(hist)
    print(f"[{e}]/[{params.epochs}], Train Loss: {train_epoch_loss:.4f}, Test Loss: {test_epoch_loss:.4f}, V-scoreL {np.mean(v_scores):.4f}")
